...
function Button({ as: Tag = 'button', children, className = '', onClick = () => {return;}, isLoading = false, ...rest }: ButtonProps) {
  ...
  return <Element disabled={isLoading} {...rest} onClick={onClick} className={`py-[14px] px-6 font-semibold bg-[#1A73E8] text-white rounded disabled:bg-gray-400 disabled: hover:bg-button-hover duration-150 flex items-center justify-center ${className} ${className.includes('px') ? '' : 'px-3'} ${className.includes('py') ? '' : 'py-2'} }`}>
    {children}
